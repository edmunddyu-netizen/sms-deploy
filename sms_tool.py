...
def send_batch_round2():
    ...
    try:
        while True:
            current_active_slots = [slot for slot in active_slots if len(slot_queues[slot]) > 0]
            ...
            for slot in ready_slots:
                if len(slot_queues[slot]) == 0:
                    continue

                item = slot_queues[slot].pop(0)
                name = item["name"]
                phone = item["phone"]
            ...
            if len(current_active_slots) > 1 and all(slot in status_dirty_slots for slot in current_active_slots):
                print_compact_next_times(next_ready, cfg, current_active_slots)
                for slot in current_active_slots:
                    status_dirty_slots.discard(slot)
            elif len(current_active_slots) == 1 and current_active_slots[0] in status_dirty_slots:
                print_compact_next_times(next_ready, cfg, current_active_slots)
                status_dirty_slots.discard(current_active_slots[0])

    except KeyboardInterrupt:
        remaining_total = sum(len(slot_queues[s]) for s in active_slots if len(slot_queues[s]) > 0)
        print("\n已手动停止第二轮发送")
        print(
            f"当前结果 -> 成功: {success} 失败: {fail} "
            f"剩余联系人: {remaining_total}"
        )
        return
